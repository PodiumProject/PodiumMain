...
function saveBlobImage(img, mid)
{
	...
	formData.append('_nonce', ajax_nonce);
		...
		success: function (response) {
		  var data = $.parseJSON(response);
			if(data.error != '')return notify('error', data.error);
				...
				notify( 'success', lang.img_saved + '. <a href="'+data.media_url+'">'+ lang.view_here +'</a>' );
			}
		},
		error: function () {
...
writeText = function(pos){
	...
	if( fsize > 60 )context.fillText(text, 80, 80);
	else context.fillText(text, 48, 48);
