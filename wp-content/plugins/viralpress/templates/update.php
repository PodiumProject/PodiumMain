...
<div id="wpbody-content">
            ...
            if ( empty( $_REQUEST['_nonce'] ) || !wp_verify_nonce( $_REQUEST['_nonce'], 'vp-admin-action-'.get_current_user_id() ) ) {
				...
				if( !empty( $_POST['envato_username'] ) && !empty( $_POST['envato_api_key'] ) && !empty( $_POST['envato_purchase_code'] ) ) {
					...
					if( $r === true ) {
						...
						echo '<div class="error"><p>'. __( sprintf( 'Failed to install license. %s', $r ) , 'viralpress' ). '</p></div>';	
					}	
				}
				else{
    ...
    ?>
        ...
        <form method="post">
            ...
            <p class="description"><?php _e( sprintf( 'Learn %s here %s how to get an api key', '<a href="https://b6444c9bc653c97975df133b789efcaf310304c1.googledrive.com/host/0B34QQcRSxhm2RTJrNHFqa0tzTTg/api_key.png" target="_blank">', '</a>' ), 'viralpress' )?></p>
