...
if( in_array( $attributes['post_type'] , array( 'news', 'list', 'audio', 'video' ) ) && !$attributes['vp_instance']->settings['list_enabled'] ) {
...
else if( in_array( $attributes['post_type'] , array( 'poll' ) ) && !$attributes['vp_instance']->settings['poll_enabled'] ) {
...
if( !empty($success) ) {
	echo 
		'<script>window.location.href = "'.home_url( '/' ).'create/?msg='.base64_encode($message.' '.__( 'View the preview', 'viralpress' ).' <a href="'.$success.'">'.__( 'here', 'viralpress' ).'</a>').'";</script>';
		exit();
}
...
if( $show_editor ){
	...
	echo get_template_html( 'editor', $attributes );
