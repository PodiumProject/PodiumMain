...
class vp_user
{
	...
	{	
		...
		$login = get_template_html( 'login', $attributes );
		$sdk = get_template_html( 'sdks',  array( 'load_fb' => 1, 'load_goo' => 1 ) );
	...
	public function vp_logout()
	{
		if( isset($_GET['logout']) ) { 
			...
			else $redirect_url = home_url( 'login?logged_out=true' );
	...
	public function vp_render_login_modal()
	{
		...
		echo '</div>';
		echo get_template_html('sdks', array( 'load_fb' => 1, 'load_goo' => 1 ) );
	}
	...
	public function render_viralpress_registration_page( $attributes, $content = null )
	{
		...
		if ( is_user_logged_in() ) {
			...
			return get_template_html( 'register', $attributes );
		}
	}
	...
	public function render_viralpress_password_lost_form()
	{
		...
		if ( is_user_logged_in() ) {
			...
			return get_template_html( 'password_lost', $attributes );
		}
	}
	...
	public function render_viralpress_password_reset_form()
	{
		...
		if ( is_user_logged_in() ) {
			...
			if ( isset( $_REQUEST['login'] ) && isset( $_REQUEST['key'] ) ) {
				...
				return get_template_html( 'password_reset', $attributes );
			} else {
	...
	{
		return home_url( '/password-lost?redirect_to='.$redirect );
	}
	...
	{	
		...
		return get_template_html( 'profile', $attributes );
	}
