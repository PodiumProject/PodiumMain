...
class vp_post
{
	...
	public function add_post()
	{
		...
		if( !validate_post_type( $post_type ) ) {
			$this->error = __( 'Invalid post type' , 'viralpress' );
			return false;
		}
		
		if( in_array( $post_type , array( 'news', 'list', 'audio', 'video', 'gallery', 'playlist' ) ) && !$vp_instance->settings['list_enabled'] ) {
		...
		else if( in_array( $post_type , array( 'poll' ) ) && !$vp_instance->settings['poll_enabled'] ) {
		...
		if( get_option( 'vp-use-category' ) == 1 ) {
			$slug = 'news';
			...
			$tt = get_term_by( 'slug', $slug, 'category' );
			if( !empty( $tt->term_id ) ) {
				$category_ids[] = (int)$tt->term_id;
				$category_ids = array_unique( $category_ids );	
			}
			else {
				$this->error = __( 'The category for "'.$post_type.'" does not exist. Please create the category first.' , 'viralpress' );
				return false;
			}
		}
		...
		if( empty($this->error) ) {
			...
			if(!empty( $post_id) ){
				...
				if( $post_status == 'draft' )$status = 'draft';
						...
						$auto_publish = get_option( 'vp-auto-publish-post' );
			...
			if( $post_type == 'quiz' && !preg_match('/^quiz/i', $post_title) ) {
				$post_title = __( 'Quiz', 'viralpress' ).': '.$post_title;	
			}
			else if( $post_type == 'polls' && !preg_match('/^poll/i', $post_title) ) {
				$post_title = __( 'Poll', 'viralpress' ).': '.$post_title;	
			}
	...
	static function render_post_entry( $atts )
	{
		...
		*/
		 ...
		 * preview requested
		 */
		if( $order == 1 && !empty( $_REQUEST['vp_preview'] ) && $current_user_is_admin ) {
			
			$preview_id = (int)$_REQUEST['vp_preview'];
			$preview_post = get_post( $preview_id );
			
			if( $preview_post instanceof WP_Post ) {
				echo '<style>.vp-preview fieldset{border:none !important;}.vp-preview legend{display: none}</style><div class="vp-clearfix"></div>';
				echo '<div class="vp-clearfix-lg"></div><div class="vp-preview"><ul id="vp-tab"><li class="active vp-op-list">';
				echo do_shortcode( '[vp_post_entry type="'.$preview_post->post_type.'" id="'.$preview_id.'" order="0"]' );
				echo '</li></ul>';
				echo '<script>jQuery(document).ready(function($){$("html,body").animate({scrollTop: $(".vp-preview").offset().top}, "fast");})</script>';
				echo '</div>';
			}
		}
		...
		if ( $child_post instanceof WP_Post ) {
			...
			if( $child_author != $post_author ) {
				//$u = new WP_User( $child_author );
				//if( !in_array( 'administrator', $u->roles ) ) return false;
			}
			...
			if( $type == 'text' || $type == 'news' ) {
				$content = '';
				echo '<div class="vp-entry" id="vp-entry-'.$child_post_id.'" data-rel="'.$child_post_id.'">';
				echo '<fieldset class="'.$cls.'">';
				list( $ptitle, $pcontent ) = display_list( $post, $child_post, $content, $show_numbers, $order );
				echo '</fieldset>';
				echo '</div>';
			}
			else if( $type == 'lists' ) {
				...
				if( !is_numeric( $vid ) ) {
					...
					$content = '<div class="vp-audio">'.$link.'</div>';
					if( !empty($link_data['caption'] ) ) $content .= '<div class="vp-media-caption">'.esc_html( htmlspecialchars_decode( $link_data['caption'] ) ).'</div>';
				...
				echo '<div class="vp-entry quiz-unchecked" id="vp-entry-'.$child_post_id.'">';
				echo '<fieldset class="'.$cls.'">';
				if( $show_numbers ){
					$list_style = get_list_style( $post->ID );
					if( $list_style == 'legend' )echo '<legend>'.$order.'</legend>';
				}
				echo '<h3 class="quiz_title" data-rel="'.$post->ID.'">';
				echo ( $list_style == 'inline' ? 
				...
				if( !empty($link) )echo '<div class="vp-quiz-img">'.$link.'</div><div class="vp-clearfix"></div>';
				
				if( !empty($child_post->post_content) )
				echo '<div class="vp-clearfix"></div>
					  <div class="entry">'.format_content( $child_post->post_content ).'</div>
					  <div class="vp-clearfix"></div>';
				
				echo __( 'Choose an answer', 'viralpress' ).'<div class="vp-clearfix"></div>';
				...
				if( !empty($answers) ){
					...
					echo '<div class="row quiz_ans_list" data-rel="'.$child_post_id.'">';
					...
					foreach( $answers as $ans ) {
						...
						echo '<div class="col-lg-'.$c.'"><div class="quiz_ans_list_obs"></div><div class="quiz_row" data-rel="'.$ans.'">';
						if(!empty($link))echo '<div class="quiz_ans_img_'.$s.'" style="background-image: url(\''.$link.'\')"></div>';
						//<img class="vp-quiz-ans-image" src="'.$link.'"/></div><div class="vp-clearfix">
						echo '<div class="quiz_ans">
				...
				if( !empty( $explain ) ) {
					...
					echo '<div class="row quiz_ans_exp">
			...
			echo '<div class="vp-entry">';
			echo '<fieldset class="vp-noborder">';
			if( !empty($title) )
			echo '<h3>'.$title.'</h3>';
			...
			if( !empty( $image ) ) {
				if( is_numeric( $image) ) {
					...
					if( !empty( $link ) ) {
						echo $link;
						if( !empty($link_data['caption'] ) ) echo '<div class="vp-media-caption">'.$link_data['caption'].'</div><div class="vp-clearfix-lg"></div>';
					}
				}
				else {
					echo $link = '<a href="'.$image.'" class="vp-list-image-link"><img src="'.$image.'" class="vp-list-image"/></a>';	
				}
			}
			
			if( !empty($desc) )
			echo '<div class="entry">'.format_content( $desc ).'</div>';
			echo '</fieldset>';
			echo '</div>';
			
		}
		
		if( $type != 'quiz' && $type != 'poll' && $type != 'post_preface' && ( $open_list || $show_numbers ) ) {
			...
			echo '<div class="vp-clearifx"></div>';
			echo '<hr/>';
			echo '<div class="vp-op-au-1 vp-op-au-5">
			...
			echo '<div class="vp-op-au-1">';
			echo get_avatar( $child_author, 60 );
			echo '</div>';
			echo '<div class="vp-op-au-2">';
			echo __( 'Submitted by', 'viralpress' ).'<br/>';
			echo vp_get_author_url( $child_author );
			echo '</div>';
			...
			if( empty( $vp_instance->settings['show_like_dislike'] ) ) {			
				...
				if( $editable ) {
					echo '<div class="vp-op-au-4"><br/>';
					echo '<i class="glyphicon glyphicon-trash vp-pointer vp_open_list_del" data-rel="'.$child_post_id.'"></i>';
					echo '</div>';
				}
				
				echo do_shortcode( '[vp_post_upvote_buttons post_id="'.$child_post_id.'"]' );
			}
			else {
				
				echo do_shortcode( '[vp_post_like_buttons post_id="'.$child_post_id.'"]' );
				if( $editable )
					echo '<div class="vp-op-au-3">
							<i class="glyphicon glyphicon-trash vp-pointer vp_open_list_del" data-rel="'.$child_post_id.'"></i>
						  </div>' ;
			}
			
			echo '<div class="vp-clearifx"></div><div class="vp-clearfix"></div>';
		}
		if( !empty( $ad_id ) &&  $order == $child_post_count ) {
			print_ad( $ad_id );
		}
		...
		//fb-comments 
		if( $type != 'quiz' && $type != 'polls' && ( $open_list || $show_numbers ) && !empty( $vp_instance->settings['comments_per_list'] ) && is_single() ) {
			echo '<div class="vp-clearfix"></div><div class="fb-comm-waypoint" data-href="'.get_the_permalink().'#'.$order.'" data-numposts="5" data-width="100%">'.__( 'Loading comments. Please wait...', 'viralpress' ).'</div><div class="vp-clearfix"></div>';
		}
	}
	...
	static function vp_score_entry()
	{
		...
		if( !empty($scores) ){
			...
			foreach( $scores as $score ) {
				...
				$data[] = array( 'min_score' => (int)$min[0], 'max_score' => (int)$max[0], 'title' => esc_js( htmlspecialchars_decode($t)), 'desc' => esc_js(htmlspecialchars_decode( $pdata->post_content ) ) , 'image' => $url );	
			}
		}
		...
		if( !empty($data) ){
			...
			if( !empty( $fixed_ans ) ) $t = esc_html( __( sprintf( 'I scored %s and got: %s on this quiz: %s' , '[myscore]', '[myresult]', get_the_title() ), 'viralpress' ) ) ;
			else $t = esc_html( __( sprintf( 'I got: %s on this quiz: %s', '[myresult]', get_the_title() ), 'viralpress' ) ) ;
			
			$tsh = $vp_instance->settings['share_quiz_force'] ? esc_html( __( sprintf( 'I have taken this quiz on %s', get_bloginfo( 'name' ) ), 'viralpress' ) ) : $t;
			
			echo "<script>var quiz_type = '".$quiz_type."';var score_sheets = '".str_replace("\\\\'", "\\'", json_encode($data))."';var score_sheet_fixed_ans = '".( (int)$fixed_ans )."';</script>";
			echo 
	...
	static function vp_poll_entry()
	{
		...
		echo '<div class="poll-feedback"></div>';
		if( empty($voted) ) {
			echo '<div class="poll_submit" style="display:none">
					<br/><br/>
			...
			echo '<p><small>'.__( 'You have submitted your vote. Thank you for your participation.' ).'</small></p>';
		...
		if( !empty( $voting_till ) ) {
			if( strtotime( $voting_till ) < time() ) {
				$voted = 1;
				echo '<div class="vp-clearfix"></div><div class="alert alert-info">'.__( 'Voting has been closed', 'viralpress' ).'</div>';
				if( empty( $p ) ) $p = json_encode( get_poll_results( $post_id) );
			}
			else echo '<div class="vp-clearfix"></div><div class="alert alert-info">'.__( 'Voting will close on', 'viralpress' ).' '.$voting_till.'</div>';
		}
		
		
		echo '<div class="poll-results-p" style="display:none"><h3>'.__( 'Poll Results', 'viralpress' ).'</h3>
	...
	public function render_editor( $attributes )
	{
		...
		$post_type = @esc_html($_POST['vp_post_type']);
		if( empty( $post_type ) || !validate_post_type( $post_type ) ){
			$this->render_html = '';
			return false;
		}
	...
	public function render_post_editor( $post_id, $copy = 0 )
	{
		...
		$list_style = get_list_style( $post->ID );
		$list_display = get_list_display( $post->ID );
		...
		foreach( $t as $tt ) {
			if( validate_post_type( $tt->name ) ) continue;
		...
		*/
		...
		foreach( $childs as $child ) {
			...
			if( $type == 'text' || $type == 'news' ){
				...
				$this->render_quiz_entry( $child_post->post_title, $child_post->post_content, $eid, $c_ans, $ans_text, $ans_imgs, $ans_ids, $l_show_numbers, $quiz_type, $type, $pid );
			}
		}
		...
		if( $type == 'quiz' ) {
			...
			if(!empty($aid)) {
				...
				foreach( $aid as $aa ) {
					...
					$this->render_score_entry( $ans_text, $ans_desc, $ans_imgs, $aa, $score_from, $score_to );
				}
			}
		...
		$t = __( 'Edit post', 'viralpress' ).' - '.get_bloginfo( 'name' );
		$this->render_html .= 'document.title="'.$t.'";$(".editor_loader").hide();$("#add_new_post_form").show();})</script>';
	}
	...
	public function render_list_entry( $title, $desc, $thumb, $l_show_numbers, $source, $pid )
	{
		...
		if( !empty($url) )$this->render_html .= 'add_thumb_image( p.parents(".more_items:first").find(".thumbnail_uploader"), "'.$thumb.'", "'.$url.'" );';
		if( !empty($pid) )$this->render_html .= 'pp.find(".entry_post_id").val("'.$pid.'");';
		if( !empty($l_show_numbers) )$this->render_html .= 'pp.find(".entry-show-num").prop("checked", true);';
		else $this->render_html .= 'pp.find(".entry-show-num").prop("checked", false);pp.removeClass("more_items_numbered");pp.find(".entry-no").html("");';
	}
	...
	public function render_hosted_media_entry( $title, $desc, $val, $l_show_numbers, $source, $type, $pid )
	{
		...
		$this->render_html .= 'p = add_new_entry( "'.$type.'" ); pp = p.parents(".more_items:first"); pp.find(".entry_text").val($("<div />").html("'.$title.'").text());pp.find(".entry_desc").val($("<div />").html("'.$desc.'").text());pp.find(".entry_source").val("'.$source.'");';
		if( !empty($url) )$this->render_html .= 'add_thumb_image( p.parents(".more_items:first").find(".thumbnail_uploader"), "'.$thumb.'", "'.$url.'", "'.$caption.'" );';
		if( !empty($pid) )$this->render_html .= 'pp.find(".entry_post_id").val("'.$pid.'");';
		if( !empty($l_show_numbers) )$this->render_html .= 'pp.find(".entry-show-num").prop("checked", true);';
		else $this->render_html .= 'pp.find(".entry-show-num").prop("checked", false);pp.removeClass("more_items_numbered");pp.find(".entry-no").html("");';
	}
	
	public function render_quiz_entry( $q, $desc, $thumb, $correct_ans, $ans, $ans_imgs, $entry_ans_post_id, $l_show_numbers, $quiz_type, $type, $pid ) 
	{
		global $vp_instance;
	...
	public function render_score_entry( $title, $desc, $thumb, $pid, $score_from, $score_to )
	{
		global $vp_instance;
		...
		$this->render_html .= 'p = add_new_entry( "results" ); pp = p.parents(".more_items_x:first"); pp.find(".entry_text").val($("<div />").html("'.$title.'").text());pp.find(".entry_desc").val($("<div />").html("'.$desc.'").text());';
		if( !empty( $score_from ) ) $this->render_html .= 'pp.find(".quiz_res_from_score").val("'.$score_from.'");pp.find(".quiz_res_to_score").val("'.$score_to.'");';
	...
	{
		echo get_template_html( 'post-comments' );	
	}
	
	static function render_my_recent_comments()
	{
		echo get_template_html( 'my-comments' );	
	}
}
