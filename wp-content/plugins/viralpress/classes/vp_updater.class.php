	...
	public function vp_download_from_envato( $default, $package, $updater )
	{
		...
		if ( ( isset( $updater->skin->plugin ) && $updater->skin->plugin === $this->plugin_slug ) ||
			...
			if ( vp_check_license() == - 1 || empty( $username ) || empty( $api_key ) || empty( $purchase_code ) ) {
				return new WP_Error( 'no_credentials', __( sprintf( 'ViralPress license is not activated. Please visit %s here %s to activate', '<a href="' . admin_url( 'admin.php?page=viralpress-update' ) . '" target="_blank">', '</a>' ), 'viralpress' ) );
			}
			...
			if ( ! isset( $result['download-purchase']['download_url'] ) ) {
				return new WP_Error( 'no_credentials', __( sprintf( 'Envato API error%s' , isset( $result['error'] ) ? ': ' . $result['error'] : '.'  ), 'viralpress' ) );
			}
