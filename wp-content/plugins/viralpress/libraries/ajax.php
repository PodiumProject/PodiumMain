...
function vp_google_auth()
{
	...
	if( $current_user ) {
		...
		if( !empty($data['picture']) ){		
			...
			$picture = download_avatar( $picture , $current_user);	
		}
	...
	if( !empty($user) ) {
		...
		if( !empty($data['picture']) ){		
			$picture = @$data['picture'];
			$picture = download_avatar( $picture , $user->ID );	
		}
...
function vp_fb_auth()
{
	...
	if( $current_user ) {
		...
		if( !empty($data['cover']['source']) ){
			...
			$cover = download_avatar( $cover, $current_user );	
		}
	...
	if( !empty($user) ) {
		...
		if( !empty($data['cover']['source']) ){
			$cover = @$data['cover']['source'];
			$cover = download_avatar( $cover, $user->ID );	
		}
...
function vp_upvote_item()
{
	...
	if( $mm == 1 ) {
		...
		do_action( 'viralpress_list_upvote_withdrawn', $post_id, $uid, $u, $d );
		output( $response );
	}
	else if( $mm == -1 && $d > 0 ) {
		$d--;
	...
	do_action( 'viralpress_list_upvoted', $post_id, $uid, $u, $d, $mm == -1 ? 1 : 0 );
...
function vp_downvote_item()
{
	...
	if( $mm == -1 ) {
		...
		$response['total_votes'] = $u - $d;
		$response['vote_withdrawn'] = 1;
		do_action( 'viralpress_list_downvote_withdrawn', $post_id, $uid, $u, $d );
		output( $response );
	}
	else if( $mm == 1 && $u > 0 ) {
		$u--;
	...
	do_action( 'viralpress_list_downvoted', $post_id, $uid, $u, $d, $mm == 1 ? 1 : 0 );
...
function vp_like_item()
{
	...
	if( $mm == 1 ) {
		...
		$response['vote_withdrawn'] = 1;
		do_action( 'viralpress_list_like_withdrawn', $post_id, $uid, $u, $d );
		output( $response );
	}
	else if( $mm == -1 && $d > 0 ) {
		$d--;
	...
	do_action( 'viralpress_list_liked', $post_id, $uid, $u, $d, $mm == -1 ? 1 : 0 );
...
function vp_dislike_item()
{
	...
	if( $mm == -1 ) {
		...
		$response['total_dislikes'] = $d;
		$response['vote_withdrawn'] = 1;
		do_action( 'viralpress_list_dislike_withdrawn', $post_id, $uid, $u, $d );
		output( $response );
	}
	else if( $mm == 1 && $u > 0 ) {
		$u--;
	...
	do_action( 'viralpress_list_disliked', $post_id, $uid, $u, $d, $mm == 1 ? 1 : 0 );
...
function vp_gif_react()
{
	...
	$data = array(
		...
		'comment_content' => ( $comment ? $comment.'<br/><br/>' : '' ).'<a class="vp-comment-link" href="'.$url.'"><img class="vp-comment-image" src="'.$url.'"/></a>',
		'comment_type' => '',
		'comment_parent' => 0,
		'user_id' => $uid,
...
function vp_load_open_list_editor()
{
	...
	$t = get_template_html( 'open_list_editor', array( 'lang' => $lang, 'vp_instance' => &$vp_instance, 'post' => get_post( $post_id ) ) );
...
function vp_meme_save()
{
	...
	else $response['media_url'] = wp_get_attachment_url( $new_id );
