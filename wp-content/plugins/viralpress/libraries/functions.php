...
function get_template_html ( $template_name, $attributes = array() )
{
	global $vp_instance;
...
function download_avatar( $url, $user_id )
{
...
function validate_post_type( $value )
{
	$valid = array('news', 'lists', 'polls', 'quiz', 'videos', 'audio', 'pins', 'gallery', 'playlist');
...
function get_video_thumb_from_url( $value, $post_id ) 
{
	...
	$title = '';
	if( $vid[1] == 'youtube' ) {
		$url = 'https://i.ytimg.com/vi/'.$vid[2].'/hqdefault.jpg';	
	}
	else {
	...
	if( !empty( $url ) ) {
		...
		$id = get_attachment_id_from_src( $src );
...
function print_post_data( $small = 0 )
{
	global $post;
...
function get_embed_code( $vid , $elem_id)
{
	...
	$w = apply_filters( 'viralpress_embed_width', $w, $w );
	$h = apply_filters( 'viralpress_embed_height', $h, $w );
...
function get_post_search_form( $edit, $hide_actions = 0 )
{
	...
	$h .= '<div class="'.( !$edit || $hide_actions ? ' vp-pull-left ' : 'vp-pull-right' ).' vp-search-form">';
	...
	$h .= '<button class="btn btn-success profile_post_search_i"><i class="glyphicon glyphicon-search vp-pointer"></button></i>';
	$h .= '</div>';	
	$h .= '</form>';
		...
		echo '<tr>
			...
			while ( $wp_query->have_posts() ) : $wp_query->the_post();
				...
				if( !empty($t[0]) ){
					...
					else if( $t[0] == 'pins' )$t = '<span class="label label-primary">'.__( 'Pins', 'viralpress' ).'</span>';
				...
				else $t = '<span class="label label-default">'.__( 'None', 'viralpress' ).'</span>';
				
				$s = get_post_status();
				$excerpt = get_the_excerpt();
				if( !empty( $excerpt ) ) $excerpt = '<br/><small style="color:gray">'.$excerpt.'</small>';
				$thumb = get_the_post_thumbnail( null, array(92, 92) );
				
				$h = '<tr class="p_'.$pid.'" data-rel="'.$pid.'">
						'.( $edit ? '<td><input type="checkbox" class="vp_post_sel" value="'.$pid.'"/></td>' : '' ).'
						<td>
							<div class="row">
								'. ( $thumb ? 
								'<div class="col-lg-2">
						...
						<td>'.$t.'</td>
						'.( $edit ? '<td class="post_status"><span class="label label-'.( $s == 'publish' ? 'success' : 'danger' ).'">'.$s.'</span></td>' : '' ).'
						<td>'.wp_count_comments( $pid )->approved.'</td>
						<td>'.get_the_time( get_option( 'date_format' ) ).'</td>
					 </tr>';
...
function display_list( $post, $child_post, $content, $show_numbers, $order, $quiz = 0 )
{
	...
	if( $show_numbers ){
		...
		$list_disp = get_list_display( $post->ID );
	}
	...
	if( preg_match( '/two_/i', $list_disp ) )echo '<div class="vp-clearfix"></div><div class="row">';
	...
	if( $list_disp == 'two_l' ) {
		...
		$tt .= '<div class="entry">'.format_content( $child_post->post_content ).'</div>';
		$tt .= '</div>';
		
		echo apply_filters( 'viralpress_two_col_left_picture_html', $tt, $post, $child_post, $content, $show_numbers, $order, $quiz = 0 );	
	}
		...
		echo apply_filters( 'viralpress_two_col_right_picture_html', $tt, $post, $child_post, $content, $show_numbers, $order, $quiz = 0 );
	}
	else{	
		$tt = '<h3 '.( $quiz ? 'quiz_title' : '' ).'>';
		$tt .= ( $list_style == 'inline' ? 
		...
		if( !empty($child_post->post_content) )
		$tt .= '<div class="vp-clearfix"></div><div class="entry">'.format_content( $child_post->post_content ).'</div>';
		if( empty( $content ) && $source_url )$tt .= '<div class="vp-media-caption">'.pretty_source( $source_url ).'</div>';
		
		echo apply_filters( 'viralpress_normal_display_list_html', $tt, $post, $child_post, $content, $show_numbers, $order, $quiz = 0 );
	}
	
	if( preg_match( '/two_/i', $list_disp ) )echo '</div>';
	
	do_action( 'viralpress_after_list_display', $post, $child_post, $content, $show_numbers, $order, $quiz );
	$html = ob_get_contents();
	ob_end_clean();
	$html = apply_filters( 'viralpress_list_display_html', $html, $post, $child_post, $content, $show_numbers, $order, $quiz  );
	echo $html;
	
	return array( $child_post->post_title, $child_post->post_content );
}
...
function get_list_style( $post_id )
{
	$style = 'legend';
...
function get_list_display( $post_id )
{
	$disp = 'one';
...
function vp_bp_add_activity( $ID )
{
	...
	if ( function_exists('bp_activity_add') ) {
		...
		$data['action'] = __( sprintf( '%s created a %s, %s', bp_core_get_userlink( $bp_uid ), $type, '<a href="'.$slug.'">'.preg_replace( '/^quiz\:|^poll\:/', '', $post->post_title ).'</a>' ), 'viralpress' );
...
function vp_format_buddypress_notifications( $action, $item_id, $secondary_item_id, $total_items, $format = 'string', $ac_name, $c_name, $id  ) 
{
	if ( 'vp_post_approved_noti' === $action ) {
		...
		if( empty( $post ) || is_wp_error( $post ) ) {
			...
			$custom_link  = $nn->guid;
			$custom_text =  __( sprintf( 'Your open list submission on the post titled "%s" has been published.', $nn->post_title ), 'viralpress' );	
		}
		else {
			$custom_title = __( 'Your submission has been published.', 'viralpress' );
			$custom_link  = $post->guid;
			$custom_text =  __( sprintf( 'Your submission titled "%s" has been published.', $post->post_title ), 'viralpress' );
		}
	}
		...
		if( empty( $link ) ) {
			...
			$custom_link  = $link;
			$custom_text =  __( sprintf( '"%s" replied to your comment titled "%s".', $name, $text ), 'viralpress' );
		}	
	}	
		...
		if( empty( $link ) ) {
			...
			$custom_link  = $link;
			$custom_text =  __( sprintf( '"%s" commented on your post titled "%s".', $name, $text ), 'viralpress' );
		}	
	}	
...
function vp_profile_screen_openlists_show()
{
	...
	if( $noti_count ){
		echo '<div class="alert alert-info">'.__( sprintf( 'You have %d new open list(s) approved since your last visit to this page', $noti_count ) , 'viralpress' ).'</div>';
    }
	
	echo '<style>.vp-entry fieldset{border:none !important;}.vp-entry legend{display: none}</style><div class="vp-clearfix"></div>';
	...
	<?php 
		...
		foreach( $posts as $p ) :
			...
			$tt .= '<div class="vp-op-bp-wrap"><hr/><div class="vp-clearfix"></div>';
			if($p['post_status'] == 'vp_open_list') {
				$tt .= '<div class="vp-pull-left">';
				$tt .= '<i class="glyphicon glyphicon-ok"></i>&nbsp;&nbsp;';
				$tt .= __( sprintf( 'This submission has been published. %sView Post%s', '<a href="'.$p['parent_post_link'].'">', '</a>' ), 'viralpress' );
				$tt .= '</div>';
			}
			else {
				$tt .= '<div class="vp-pull-left">';
				$tt .= '<i class="glyphicon glyphicon-hourglass"></i>&nbsp;&nbsp;';
				$tt .= __( sprintf( 'This submission is pending. %sView Post%s', '<a href="'.$p['parent_post_link'].'">', '</a>' ), 'viralpress' );
				$tt .= '</div>';
			}
			if( $theuser == $viewer ) {
			...
			$tt .= ob_get_contents();
			ob_end_clean();
			$tt .= '</li></ul></div><div class="vp-clearfix-lg"></div>';
		...
		$args = array(
			...
			'base'               => '%_%',
			'format'             => '?paged=%#%',
			'add_args'           => array( 'post_status' => @$_REQUEST['post_status'] ),
			'type'				 => 'plain',
...
function vp_get_author_url( $child_author )
{
	...
	else $r .= '<a href="'.get_the_author_link( $child_author ).'">'.get_the_author_meta( 'user_nicename' , $child_author ).'</a>';	
...
function vp_get_pending_open_list( $uid = 0, $paged = 1, $per_page = 10, $orderby, $order ) 
{
	...
	foreach( $p as $pp ) {
		...
		if( !empty( $nn ) ) {
			if( preg_match( '/\?/', $nn->guid ))$plink = $nn->guid.'&vp_preview='.$ppp['ID'];
			else $plink = $nn->guid.'?vp_preview='.$ppp['ID'];
		...
		$ppp['post_title'] = '<a href="'.$plink.'">'.( $ppp['post_title'] ? $ppp['post_title'] : __( 'View', 'viralpress' ) ).'</a>';
		$ppp['author'] = vp_get_author_url( $ppp['post_author'] );
		$ppp['parent_post'] = '<a href="'.$plink.'">'.$ptitle.'</a>';
...
function vp_enqueue_script_meme_page()
{
	...
	wp_localize_script( 'vp-meme-js', 'lang', $meme_lang );
	wp_localize_script( 'vp-meme-js', 'ajaxurl', admin_url( 'admin-ajax.php' ) );
	wp_localize_script( 'vp-meme-js', 'ajax_nonce', wp_create_nonce( 'vp-ajax-action-'.get_current_user_id() ) );
}
?>