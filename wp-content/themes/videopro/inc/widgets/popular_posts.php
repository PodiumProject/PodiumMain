...
class videopro_Popular_posts extends WP_Widget
{
	function videopro_popular_posts()
	{
		...
		parent::__construct('popular_posts_id', 'VideoPro - Popular Posts', $options);
	}
	...
	function form($instance)
	{
		...
		$html  .= '<select name="' . $this->get_field_name('order_by') . '">
						...
						<option value="featured"' . $featured . '>' . esc_html__('Featured', 'videopro') . '</option>
					</select>';
		$html  .= '</p>';
	...
	function widget($args, $instance)
	{
		...
		$title 					= $instance['title'] != '' ? $instance['title'] : esc_html__('Headline', 'videopro');
