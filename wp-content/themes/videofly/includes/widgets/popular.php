...
class widget_popular extends WP_Widget {
    ...
    function widget_popular() {
        $widget_ops = array( 'classname' => 'widget_popular' , 'description' => esc_html__( " Get popular posts." , 'videofly' ) );
        parent::__construct( 'widget_touchsize_popular' , esc_html__( "Most popular posts" , 'videofly' ) , $widget_ops );
    }
    ...
    function widget( $args , $instance ){
		...
		foreach( $storagePosts as $time => $posts ){
			...
			if( !empty($posts) ){
				foreach( $posts as $key => $post ){
					$storageShared[$time][$key] = (int)get_post_meta($post->ID, 'ts-social-count', true) + (int)get_post_meta($post->ID, 'ts_article_views', true) + (int)get_post_meta($post->ID, '_touchsize_likes', true);
				}
        ...
        foreach( $storageShared as $time => $timePosts ): ?>
        	...
        	<ul class="widget-items row <?php echo ' widget-columns-' . $columns; echo($image == 'y' ? ' widget-has-image' : '') ?>">
	        	<?php foreach( $timePosts as $keyStoragePosts => $countShared ): ?>
					...
					if( get_post_thumbnail_id($storagePosts[$time][$keyStoragePosts]->ID) ){
						$post_img = wp_get_attachment_image(get_post_thumbnail_id($storagePosts[$time][$keyStoragePosts]->ID) , 'vdf_grid' , '');
						$cnt_a1 = ' href="' . get_permalink($storagePosts[$time][$keyStoragePosts]->ID) . '"';
						$cnt_a2 = ' href="' . get_permalink($storagePosts[$time][$keyStoragePosts]->ID) . '#comments"';
						$cnt_a3 = ' class="entry-img" href="' . get_permalink($storagePosts[$time][$keyStoragePosts]->ID) . '"';
					...
					$article_date = get_the_date('', $storagePosts[$time][$keyStoragePosts]->ID); ?>
	                        	...
	                        	<div class="entry-content">
	                                <h4 class="title">
	                                	<a <?php echo vdf_var_sanitize($cnt_a1); ?>>
											<?php echo vdf_var_sanitize($storagePosts[$time][$keyStoragePosts]->post_title); ?>
										</a>
									</h4>
												...
												<li class="red-comments">
												    <a <?php echo vdf_var_sanitize($cnt_a2); ?>>
												        ...
												        <span class="comments-count">
												            <?php echo vdf_var_sanitize($storagePosts[$time][$keyStoragePosts]->comment_count) . ' '; ?>
												        </span>
												    </a>
												</li>
											<?php endif; ?>
											<?php if( $likes == 'y' ) : ?>
												<?php touchsize_likes($storagePosts[$time][$keyStoragePosts]->ID, '<li class="meta-likes">', '</li>', true, false); ?>
											<?php endif; ?>
											<?php if( $views == 'y' ) : ?>
												<?php vdf_get_views($storagePosts[$time][$keyStoragePosts]->ID, '<li class="meta-views"><i class="icon-views"></i> ', '</li>'); ?>
											<?php endif; ?>
										</ul>
									</div>
	                        	</div>
	    ...
	    <div class="ts-tab-container" data-display="horizontal">
		    <ul class="nav nav-tabs" role="tablist">
		    	<li class="active">
		    		<a href="#<?php echo 'ts-last-week-'. $this->id ?>" role="tab" ><?php esc_html_e('Last week', 'videofly'); ?></a>
		    	</li>
		    	<li>
		    		<a href="#<?php echo 'ts-last-month-'. $this->id ?>" role="tab" ><?php esc_html_e('Last month', 'videofly'); ?></a>
		    	</li>
		    	<li>
		    		<a href="#<?php echo 'ts-all-time-'. $this->id ?>" role="tab" ><?php esc_html_e('All time', 'videofly'); ?></a>
		    	</li>
		    </ul>
		    <div class="tab-content">
		    	<div class="tab-pane active" id="<?php echo 'ts-last-week-'. $this->id ?>">
		    		<?php echo vdf_var_sanitize($storageHtml['W']) ?>
		    	</div>
		    	<div class="tab-pane" id="<?php echo 'ts-last-month-'. $this->id ?>">
		    		<?php echo vdf_var_sanitize($storageHtml['monthnum']) ?>
		    	</div>
		    	<div class="tab-pane" id="<?php echo 'ts-all-time-'. $this->id ?>">
		    		<?php echo vdf_var_sanitize($storageHtml['all']) ?>
		    	</div>
		   	</div>
		</div>
