...
if ( $youtube_api = kvalue( $api_settings, 'yt_api') )
{
	$options['yt_video'] = array(
								'regex' => '#((?<=v=)[a-zA-Z0-9-]+(?=&)|(?<=v\/)[^&\n]+|(?<=v=)[^&\n]+|(?<=youtu.be/)[^&\n]+)#',
								'link'	=> 'https://www.googleapis.com/youtube/v3/videos?id={id}&part=snippet,contentDetails,statistics&key=' . $youtube_api,
								'type' =>'/(?:youtube(?:-nocookie)?\.com\/watch\?v=[a-zA-Z0-9]+)/',
								'source' => 'youtube',
							);
	
	$videos_limit = kvalue( $api_settings, 'yt_videos_limit', 10 );

	$options['yt_playlist'] = array(
								'regex' => '/([A-Za-z0-9_-]+)$/',
								'link'	=> "https://www.googleapis.com/youtube/v3/playlistItems?playlistId={id}&part=snippet&maxResults={$videos_limit}&key=" . $youtube_api,
								'type' =>'/(?:youtube(?:-nocookie)?\.com\/playlist\?list=[A-Za-z0-9_-]+)|(?:youtube(?:-nocookie)?\.com\/view_play_list\?p=[A-Za-z0-9_-]+)/',
								'source' => 'youtube',
							);
	
	$playlists_limit = kvalue( $api_settings, 'yt_playlists_limit', 10 );

	$options['yt_channel'] = array(
								'regex' => '/([A-Za-z0-9_-]+)$/',
								'link'	=> 'https://www.googleapis.com/youtube/v3/playlists?part=id,snippet,contentDetails&maxResults=' . $playlists_limit . '&channelId={id}&key=' . $youtube_api,
...
$data['youtube'] =  array('source'=>'youtube','id'=>'id','thumb'=>'high','title'=>'title','desc'=>'description','duration'=>'duration','views'=>'viewCount','author'=>'channelTitle','rating'=>'likeCount','tags'=>'', 'hd' => true);
